...
`dl-fedora 36 silverblue` : downloads the Fedora Silverblue iso

`dl-fedora respin kde` : downloads the latest KDE Live respin

`dl-fedora 35 server --arch aarch64` : will download the Server iso for armv8

`dl-fedora --run 36` : will download Fedora Workstation and boot the Live image with qemu-kvm.
